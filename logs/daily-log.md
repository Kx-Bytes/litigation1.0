...
- Shipped B5: `app/services/confidence.py` — pure pipeline confidence band computation. Three signals: `strong_chunks` (count ≥ 0.65 similarity), `jurisdiction_match_rate` (exact match fraction), `verification_rate` (verified / total cited). Rules: "high" = strong ≥ 4 + jur_rate ≥ 0.5 + verif_rate ≥ 0.75; "medium" = strong ≥ 2 + verif_rate ≥ 0.5; "low" otherwise; "refused" passthrough. Pipeline band overrides model self-report. 35/35 unit tests passing. 144/144 total (zero regressions).
- Shipped B6: `POST /api/v1/query` fully wired. Pipeline: persist Query row → embed (voyage-3-large) → retrieve (jurisdiction + date range) → generate (cite-or-refuse) → verify (B4) → compute_confidence_band (B5) → build QueryResponse → persist QueryResult (best-effort, error-safe). Refusal path returns `confidence_band="refused"` + `refusal` dict, null `risk_assessment`. Citation strings resolved from retrieved_map for `risk_factors`, `comparable_cases`, and `dropped_claims`. `latency_ms` measured end-to-end. 14/14 endpoint tests passing. 158/158 total (zero regressions). **Day 7 milestone hit: working `POST /api/v1/query` endpoint.**
